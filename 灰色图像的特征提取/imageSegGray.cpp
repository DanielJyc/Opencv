...
/************************************************************************/
void getContoursPixel(vector<vector<Point>> &contours, Mat srcImg,Vector<Mat> &subRegions)
{
	...
	for (int i = 0; i < contours.size(); i++)
	{
		...
		Mat contourRegion;
		Mat imageROI;
		srcImg.copyTo(imageROI, mask); // 'image' is the image you used to compute the contours.
		contourRegion = imageROI(roi);
		subRegions.push_back(contourRegion);
	}
//	cv::imshow("sub1", subRegions[contours.size()-1]);  //最大区域
...
void getFireFeatures(Mat srcImg, vector<vector<Point>> &contours, Mat &thresholdedImg)
{
	...
	getContoursPixel(contours, srcImg, subRegionsSrcImg);
	...
	getContoursPixel(contours, whiteImgFromSrcImg, subRegionsThresholdedImg);
	Mat biggestContourThresholdedImg = subRegionsThresholdedImg[contours.size() - 1];
	imshow("biggestContourThresholdedImg", biggestContourThresholdedImg);  
	...
	fireConers = getCountOfFireConers(biggestContourThresholdedImg);//4、尖角个数（最大火灾区域）
	//***5、变化率。带补充！！！！！！！！！
...
void getContours(Mat srcImg, Mat thresholdedImg)
{
	...
	findContours(image,  //image将变为含有轮廓的形状
